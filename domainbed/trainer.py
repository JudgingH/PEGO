...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    for step in range(n_steps):
        ...
        if step % checkpoint_freq == 0:
            ...
            # save model
            records_temp = Q(records)
            records_best = records_temp.argmax("train_out")["step"]
            records_now = records_temp.argmax("step")["step"]
            # print(records_now, records_best)
            if records_now == records_best:

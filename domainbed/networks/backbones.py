...
from . import vit_image, clip_model, PlainMultiHeadAttention, lora
...
def get_backbone(name, preserve_readout, pretrained, peft_hp, lora_nums):
    if not pretrained:
    ...
    if name == "resnet18":
        ...
        n_outputs = network.output_dim
    elif name == "clip_vit-b16_lora":
        network = clip_imageencoder("ViT-B/16")
        for module in network.transformer.resblocks:
            new_module = PlainMultiHeadAttention.PlainMultiHeadAttention(embed_dim=768, num_heads=12)
            new_module.set_parameters(module.attn)
            module.attn = new_module
        network = lora.LoRA_ViT_timm(network, r=peft_hp, lora_nums=lora_nums)
        n_outputs = network.output_dim

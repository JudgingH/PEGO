...
class LoRA_ViT(nn.Module):
    ...
    def __init__(self, vit_model: ViT, r: int, num_classes: int = 0, lora_layer=None):
        ...
        assert r > 0
        base_vit_dim = vit_model.transformer.resblocks[0].attn.proj_q.in_features
        dim = base_vit_dim
        if lora_layer:
...
class _LoRA_qkv_timm(nn.Module):
    ...
    def __init__(
        ...
        self.lora_nums = len(linear_a_q)

    def forward(self, x):
        qkv = self.qkv(x)  # B,N,3*org_C
        device = x.device
        for i in range(self.lora_nums):
            new_q = self.linear_b_q[i].to(device)(self.linear_a_q[i].to(device)(x))
            new_v = self.linear_b_v[i].to(device)(self.linear_a_v[i].to(device)(x))
            qkv[:, :, : self.dim] += new_q
            qkv[:, :, -self.dim :] += new_v
        return qkv
...
class LoRA_ViT_timm(nn.Module):
    def __init__(self, vit_model: timm_ViT, r: int, num_classes: int = 0, lora_layer=None, lora_nums=3):
        ...
        for t_layer_i, blk in enumerate(vit_model.transformer.resblocks):
            ...
            w_a_linear_q, w_b_linear_q, w_a_linear_v, w_b_linear_v = nn.ModuleList(), nn.ModuleList(), nn.ModuleList(), nn.ModuleList()
            for _ in range(lora_nums):
                w_a_linear_q.append(nn.Linear(self.dim, r, bias=False))
                w_b_linear_q.append(nn.Linear(r, self.dim, bias=False))
                w_a_linear_v.append(nn.Linear(self.dim, r, bias=False))
                w_b_linear_v.append(nn.Linear(r, self.dim, bias=False))
            self.w_As.append(w_a_linear_q)
            self.w_Bs.append(w_b_linear_q)
            self.w_As.append(w_a_linear_v)
            self.w_Bs.append(w_b_linear_v)
        ...
        self.reset_parameters()
        self.lora_vit = vit_model
        self.output_dim = vit_model.output_dim

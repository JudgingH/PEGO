...
from domainbed.networks.clip_model import convert_weights
...
class PEGO(Algorithm):
    """
    Parameter-Efficient Group with Orthogonal Regularization
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PEGO, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.network = nn.Sequential(self.featurizer, self.classifier)
        convert_weights(self.network)

        print('trainable paras:', sum(p.numel() for p in self.featurizer.parameters() if p.requires_grad))
        self.optimizer = get_optimizer(
            hparams["optimizer"],
            self.network.parameters(),
            lr=self.hparams["lr"],
        ...
        self.ld = hparams.ld
        self.lora_nums = hparams.lora_nums
    ...
    def update(self, x, y, **kwargs):
        ...
        loss_c = F.cross_entropy(self.classifier(f), all_y)

        loss_o = 0
        for block in self.featurizer.network.lora_vit.transformer.resblocks:
            lora_qkv = block.attn.qkv
            w = lora_qkv.qkv.weight
            q = w[:lora_qkv.qkv.in_features, :]
            v = w[-lora_qkv.qkv.in_features:, :]
            for i in range(self.lora_nums):
                a_q, b_q = lora_qkv.linear_a_q[i].weight, lora_qkv.linear_b_q[i].weight
                ab_q = torch.mm(b_q, a_q)
                a_v, b_v = lora_qkv.linear_a_v[i].weight, lora_qkv.linear_b_v[i].weight
                ab_v = torch.mm(b_v, a_v)

                loss_o += torch.abs(torch.mm(q, ab_q.T)).sum() + torch.abs(torch.mm(v, ab_v.T)).sum()
                # loss_o += torch.abs(torch.mm(q.T, ab_q)).sum() + torch.abs(torch.mm(v.T, ab_v)).sum()
                # loss_o += (torch.mm(q.T, ab_q)**2).sum() + (torch.mm(v.T, ab_v)**2).sum()

                for j in range(i+1, self.lora_nums):
                    a_q_j, b_q_j = lora_qkv.linear_a_q[j].weight, lora_qkv.linear_b_q[j].weight
                    ab_q_j = torch.mm(b_q_j, a_q_j)
                    a_v_j, b_v_j = lora_qkv.linear_a_v[j].weight, lora_qkv.linear_b_v[j].weight
                    ab_v_j = torch.mm(b_v_j, a_v_j)
                    loss_o += torch.abs(torch.mm(ab_q, ab_q_j.T)).sum() + torch.abs(torch.mm(ab_v, ab_v_j.T)).sum()
                    # loss_o += torch.abs(torch.mm(ab_q.T, ab_q_j)).sum() + torch.abs(torch.mm(ab_v.T, ab_v_j)).sum()
                    # loss_o += (torch.mm(ab_q.T, ab_q_j)**2).sum() + (torch.mm(ab_v.T, ab_v_j)**2).sum()
        ...
        return {"loss_c": loss_c.item(), "loss_o": loss_o.item()}
